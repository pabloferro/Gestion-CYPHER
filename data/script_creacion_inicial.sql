...
CREATE TABLE CIPHER.BONOCONSULTA(
	...
	BONC_TURNO INT REFERENCES CIPHER.TURNO
)
...
INSERT INTO CIPHER.TURNO(TURN_CODIGO,TURN_AFILIADO,TURN_FECHA,TURN_MEDICO, TURN_ESP)
	SELECT DISTINCT Turno_Numero,
	 (SELECT TOP 1 AFIL_NROAFILIADO FROM CIPHER.AFILIADO WHERE AFIL_DOCUMENTO=Paciente_Dni),
	 Turno_Fecha,
	 (SELECT MED_CODIGO FROM CIPHER.MEDICO WHERE MED_DOCUMENTO=Medico_Dni),
	 Especialidad_Codigo
...
--Carga de todos los bonos consulta, el afiliado que consumio y turno se lo deja en null 
INSERT INTO CIPHER.BONOCONSULTA(BONC_CODIGO,BONC_FECHAIMPRESION,BONC_AFILIADOCONSUMIO,BONC_TURNO,BONC_COMPRA)
	SELECT DISTINCT Bono_Consulta_Numero,Bono_Consulta_Fecha_Impresion,NULL, NULL,(SELECT top 1 COMP_CODIGO FROM CIPHER.COMPRA WHERE COMP_AFILIADO=(SELECT TOP 1 AFIL_NROAFILIADO FROM CIPHER.AFILIADO WHERE Paciente_Dni=AFIL_DOCUMENTO) AND COMP_FECHA=Compra_Bono_Fecha )
...
BEGIN
	SELECT CONVERT(TIME, TURN_FECHA) 'Hora', AFIL_NROAFILIADO 'Nro Afiliado',
		   AFIL_APELLIDO + ', ' + AFIL_NOMBRE AS  'Nombre Afiliado' , TURN_CODIGO 'Código'
...
BEGIN
	DECLARE @TURNO INT
	IF CIPHER.AFILIADOACTIVO(@AFILIADO) = 0
		RETURN -2
	IF EXISTS (SELECT *
			   FROM CIPHER.TURNO
			   WHERE TURN_MEDICO = @MEDICO AND
					 ...
					 DATEADD(dd, 0, DATEDIFF(dd, 0, TURN_FECHA)) = @FECHA AND 
					 (CIPHER.TURNOCANCELADO(TURN_CODIGO) = 0))
		SET @TURNO =  (SELECT TOP 1 TURN_CODIGO
			   FROM CIPHER.TURNO
			   WHERE TURN_MEDICO = @MEDICO AND
			         TURN_AFILIADO = @AFILIADO AND
					 DATEADD(dd, 0, DATEDIFF(dd, 0, TURN_FECHA)) = @FECHA AND 
					 (CIPHER.TURNOCANCELADO(TURN_CODIGO) = 0))
	IF EXISTS (SELECT * FROM CIPHER.REGISTROLLEGADA WHERE REGLLEG_TURNO = @TURNO)
		RETURN -3
	ELSE
		RETURN @TURNO
	RETURN -1
...
CREATE PROCEDURE CIPHER.REGISTRARLLEGADA(@AFILIADO INT, @TURNO INT, @BONO INT, @FECHA DATETIME)
AS
BEGIN
	UPDATE CIPHER.BONOCONSULTA
...
CREATE PROCEDURE CIPHER.CANCELARTURNOMEDICOTODOS(@MEDICO INT, @MOTIVO NVARCHAR(255))
AS
BEGIN
	--BORRO LOS TURNOS DISPONIBLES (AGENDA)
	DELETE FROM CIPHER.TURNO_DISPONIBLE
	WHERE TUDI_MEDICO = @MEDICO
	--CANCELO LOS TURNOS YA ASIGNADOS
	INSERT INTO CIPHER.CANCELACIONTURNO (CATU_TURNO, CATU_TIPO, CATU_MOTIVO)
	SELECT TURN_CODIGO, 4, @MOTIVO
	FROM TURNO 
...
BEGIN
	...
	--NO EXISTE EL BONO
	IF NOT EXISTS (SELECT * FROM CIPHER.BONOCONSULTA WHERE BONC_CODIGO = @BONOCONSULTA)
		RETURN 0
...
GO
CREATE FUNCTION	CIPHER.AFILIADOBONOFARMACIAVALIDO(@AFILIADO INT, @BONOFARMACIA INT, @FECHAHOY DATETIME) RETURNS BIT
AS
BEGIN
	DECLARE @PLAN_COMPRA INT, @AFILIADO_COMPRA INT, @PLAN_AFILIADO INT, @FECHA_COMPRA DATETIME, @CONSUMIDO INT
	...
	SELECT @PLAN_COMPRA = COMP_PLAN, @AFILIADO_COMPRA = COMP_AFILIADO, @FECHA_COMPRA = COMP_FECHA, @CONSUMIDO = BONF_AFILIADOCONSUMIO
	FROM CIPHER.COMPRA JOIN CIPHER.BONOFARMACIA ON COMP_CODIGO = BONF_COMPRA
	WHERE BONF_CODIGO = @BONOFARMACIA
	--NO EXISTE EL BONO
	IF NOT EXISTS (SELECT * FROM CIPHER.BONOFARMACIA WHERE BONF_CODIGO = @BONOFARMACIA)
		RETURN 0
	--EL PLAN NO COINCIDE CON EL DE COMPRA
	IF @PLAN_COMPRA <> @PLAN_AFILIADO
		RETURN 0
...
CREATE PROCEDURE CIPHER.CONSUMIRBONOFARMACIA(@AFILIADO INT, @BONO INT)
AS
BEGIN
	UPDATE CIPHER.BONOFARMACIA
	SET BONF_AFILIADOCONSUMIO = @AFILIADO
	WHERE BONF_CODIGO = @BONO
END
...
CREATE FUNCTION CIPHER.MEDICOTIENEAGENDA(@MEDICO INT)
RETURNS BIT
AS
BEGIN
	IF EXISTS (SELECT * FROM CIPHER.AGENDA WHERE AGEN_MEDICO = @MEDICO)
		RETURN 1
	IF EXISTS (SELECT * FROM CIPHER.AGENDA_DIA WHERE AGDI_MEDICO = @MEDICO)
		RETURN 1
	RETURN 0
END
...
--ESTADISTICAS
CREATE FUNCTION CIPHER.FECHAANOYSEMESTRE(@FECHA DATETIME, @ANO INT, @SEMESTRE INT) RETURNS BIT
AS
BEGIN
  IF (DATEPART(yyyy, @FECHA) = @ANO and
      DATEPART(mm, @FECHA) > 6*(@SEMESTRE-1) AND
      DATEPART(mm, @FECHA) <= 6*(@SEMESTRE))
      RETURN 1
  RETURN 0
END
...
create function CIPHER.enElMismoAnoYMes(@fecha1 Datetime, @ANO INT, @MES INT)
returns bit
as
begin
if(YEAR(@fecha1)= @ANO and MONTH(@fecha1)=@MES)
	return 1
return 0
end
...
CREATE FUNCTION CIPHER.BonosFVencidosMes(@HOY DATETIME, @AFILIADO INT, @ANO INT, @MES INT) RETURNS INT
AS
BEGIN
	RETURN  (select count(*) 
			 from CIPHER.COMPRA join CIPHER.BONOFARMACIA on 
				  BONF_COMPRA=COMP_CODIGO and COMP_AFILIADO=@AFILIADO and 
				  BONF_AFILIADOCONSUMIO is null
				  where BONF_FECHAVENCIMIENTO<@HOY and
						CIPHER.enElMismoAnoYMes(BONF_FECHAVENCIMIENTO, @ANO, @MES) = 1)
	
END

GO

create procedure CIPHER.top5BonosFVencidos(@HOY DATETIME, @ANO INT, @SEMESTRE INT)
as
begin
select top 5 
	(select count(*) 
			 from CIPHER.COMPRA join CIPHER.BONOFARMACIA on 
				  BONF_COMPRA=COMP_CODIGO and COMP_AFILIADO=AFIL_NROAFILIADO and 
				  BONF_AFILIADOCONSUMIO is null
				  where BONF_FECHAVENCIMIENTO<@HOY and
						CIPHER.FECHAANOYSEMESTRE(BONF_FECHAVENCIMIENTO, @ANO, @SEMESTRE) = 1) as 'semestre',
	AFIL_NROAFILIADO 'Nro',
	AFIL_NOMBRE 'Afiliado',
	AFIL_APELLIDO 'Apellido',
	CIPHER.BonosFVencidosMes(@HOY, AFIL_NROAFILIADO, @ANO, 1 * @SEMESTRE) AS '1',
	CIPHER.BonosFVencidosMes(@HOY, AFIL_NROAFILIADO, @ANO, 2 * @SEMESTRE) AS '2',
	CIPHER.BonosFVencidosMes(@HOY, AFIL_NROAFILIADO, @ANO, 3 * @SEMESTRE) AS '3',
	CIPHER.BonosFVencidosMes(@HOY, AFIL_NROAFILIADO, @ANO, 4 * @SEMESTRE) AS '4',
	CIPHER.BonosFVencidosMes(@HOY, AFIL_NROAFILIADO, @ANO, 5 * @SEMESTRE) AS '5',
	CIPHER.BonosFVencidosMes(@HOY, AFIL_NROAFILIADO, @ANO, 6 * @SEMESTRE) AS '6'
from CIPHER.AFILIADO 
order by 1 desc
end
...
CREATE FUNCTION CIPHER.BonosUtilizadosDeOtro(@AFILIADO INT, @ANO INT, @MES INT) RETURNS INT
AS
BEGIN
	RETURN  (select count(BONC_AFILIADOCONSUMIO) 
			 from CIPHER.BONOCONSULTA join CIPHER.COMPRA on 
				  BONC_COMPRA=COMP_CODIGO and BONC_AFILIADOCONSUMIO = @AFILIADO 
				  where BONC_AFILIADOCONSUMIO IS NOT NULL AND
						COMP_AFILIADO <> @AFILIADO AND
					    CIPHER.enElMismoAnoYMes(COMP_FECHA, @ANO, @MES) = 1)
END

GO

create procedure CIPHER.top10AfiliadosBonosUtilizadosDeOtro(@ANO INT, @SEMESTRE INT)
as
begin
select top 10 
	(select count(BONC_AFILIADOCONSUMIO) 
	 from CIPHER.BONOCONSULTA join CIPHER.COMPRA on 
		  BONC_COMPRA=COMP_CODIGO and BONC_AFILIADOCONSUMIO = AFIL_NROAFILIADO 
	 where BONC_AFILIADOCONSUMIO IS NOT NULL AND
		   COMP_AFILIADO <> AFIL_NROAFILIADO AND
		   CIPHER.FECHAANOYSEMESTRE(COMP_FECHA, @ANO, @SEMESTRE) = 1) as 'semestre',
	AFIL_NROAFILIADO 'Nro',
	AFIL_NOMBRE 'Afiliado',
	AFIL_APELLIDO 'Apellido',
	CIPHER.BonosUtilizadosDeOtro(AFIL_NROAFILIADO, @ANO, 1 * @SEMESTRE) AS '1',
	CIPHER.BonosUtilizadosDeOtro(AFIL_NROAFILIADO, @ANO, 2 * @SEMESTRE) AS '2',
	CIPHER.BonosUtilizadosDeOtro(AFIL_NROAFILIADO, @ANO, 3 * @SEMESTRE) AS '3',
	CIPHER.BonosUtilizadosDeOtro(AFIL_NROAFILIADO, @ANO, 4 * @SEMESTRE) AS '4',
	CIPHER.BonosUtilizadosDeOtro(AFIL_NROAFILIADO, @ANO, 5 * @SEMESTRE) AS '5',
	CIPHER.BonosUtilizadosDeOtro(AFIL_NROAFILIADO, @ANO, 6 * @SEMESTRE) AS '6'
from CIPHER.AFILIADO 
order by 1 desc
end
...
CREATE FUNCTION CIPHER.EspecialidadBonoFMes(@ESP INT, @ANO INT, @MES INT) RETURNS INT
AS
BEGIN
	RETURN  (select count(*) 
			 from CIPHER.BONOFARMACIA join CIPHER.TURNO on 
				  BONF_ATENCION=TURN_CODIGO and TURN_ESP = @ESP
				  where CIPHER.enElMismoAnoYMes(TURN_FECHA, @ANO, @MES) = 1)
END

GO

create procedure CIPHER.top5EspecialidadBonoF(@ANO INT, @SEMESTRE INT)
as
begin
select top 5 
	(select count(*) 
			 from CIPHER.BONOFARMACIA join CIPHER.TURNO on 
				  BONF_ATENCION=TURN_CODIGO and TURN_ESP = ESP_CODIGO
		     where CIPHER.FECHAANOYSEMESTRE(TURN_FECHA, @ANO, @SEMESTRE) = 1) as 'semestre',
	ESP_DESC 'Especialidad',
	CIPHER.EspecialidadBonoFMes(ESP_CODIGO, @ANO, 1 * @SEMESTRE) AS '1',
	CIPHER.EspecialidadBonoFMes(ESP_CODIGO, @ANO, 2 * @SEMESTRE) AS '2',
	CIPHER.EspecialidadBonoFMes(ESP_CODIGO, @ANO, 3 * @SEMESTRE) AS '3',
	CIPHER.EspecialidadBonoFMes(ESP_CODIGO, @ANO, 4 * @SEMESTRE) AS '4',
	CIPHER.EspecialidadBonoFMes(ESP_CODIGO, @ANO, 5 * @SEMESTRE) AS '5',
	CIPHER.EspecialidadBonoFMes(ESP_CODIGO, @ANO, 6 * @SEMESTRE) AS '6'
from CIPHER.ESPECIALIDAD 
order by 1 desc
end
